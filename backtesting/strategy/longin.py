...
class LongIn:
    ...
    def get_a(self):
        ...
        # sma rate condition
        sma_rate_condition = (Formula.ema(self.close, 200) - Formula.ema(self.close[:-1], 200)) > 0 and (Formula.sma(self.close, 200) - Formula.sma(self.close[:-1], 200)) > 0

        # sma condition
        sma_condition = Formula.ema(self.close, 20) > Formula.ema(self.close, 60) > Formula.ema(self.close, 120) > Formula.ema(self.close, 200) + 0.2 and Formula.ema(self.close, 60) > Formula.ema(self.close, 180) and Formula.sma(self.close, 120) > Formula.sma(self.close, 200)

        # macd and 3macd condition
        macd_condition = Formula.macd(self.close, 12, 26, 9) < 0.05 and Formula.macd(self.close, 36, 78, 27) > 0.3
        ...
        if self.highest_volume2_high == 0:
            ...
            if self.highest_volume2_high > self.highest_volume1_high:
                price_condition = price_condition and self.low.iloc[-1] + (self.high.iloc[-1] - self.low.iloc[-1]) * 0.5 > self.highest_volume2_high
            else:
                price_condition = price_condition and self.close.iloc[-1] > self.highest_volume1_high
        ...
        print("ema 120: ", Formula.ema(self.close, 120))
        print("ema 200: ", Formula.ema(self.close, 200))
