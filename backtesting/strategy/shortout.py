...
class ShortOut:
    ...
    def is_short_out(self):
        ...
        spike_condition2_3 = self.close.iloc[-1] >= self.low.iloc[-1] + (self.high.iloc[-1] - self.low.iloc[-1]) * 0.7 and self.close.iloc[-1] > self.open.iloc[-1]
        ...
        # print("price_condition: ", price_condition, ", end_of_day: ", end_of_day_condition, ", sma_condition: ", sma_condition, ", spike condition 2: ", spike_condition2, ", spike condition1: ", spike_condition1)
    ...
    def get_a(self):
        ...
        spike_condition2_1 = self.volume_spike
        spike_condition2_2 = (self.close.iloc[-1] - self.close.iloc[-3]) / 3 <= -1  # tan(-pi/4) = -1
        spike_condition2_3 = self.close.iloc[-1] >= self.low.iloc[-1] + (
                    self.high.iloc[-1] - self.low.iloc[-1]) * 0.7 and self.close.iloc[-1] > self.open.iloc[-1]
        spike_condition2 = (spike_condition2_1 and spike_condition2_2) or (
                    spike_condition2_1 and spike_condition2_3) or (spike_condition2_2 and spike_condition2_3)

        # 破线拐头出点信号
        sma_condition = self.low.iloc[-1] + (self.high.iloc[-1] - self.low.iloc[-1]) * 0.382 >= Formula.sma(self.close,
                                                                                                            20) and \
                        self.close.iloc[-1] > Formula.ema(self.close, 20)
        ...
        print("volume spike: ", self.volume_spike)
