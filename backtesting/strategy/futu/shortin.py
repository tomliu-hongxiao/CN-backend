...
class ShortIn:
    ...
    def is_short_in(self):
        ...
        # sma condition
        sma_condition = Formula.ema(self.close, 20) < Formula.ema(self.close, 60) < Formula.ema(self.close, 120) and Formula.ema(self.close, 60) < Formula.ema(self.close, 200) + 0.2

        # macd condition
        macd_condition = Formula.macd(self.close, 12, 26, 9) > 0.03 and Formula.macd(self.close, 36, 78, 27) < -0.1
    ...
    def get_a(self):
        ...
        # macd condition
        macd_condition = Formula.macd(self.close, 12, 26, 9) > 0 and Formula.macd(self.close, 36, 78, 27) < -0.1
        ...
        if self.short_out_low is not None:
            if self.close.iloc[-1] < self.short_out_low:
                ...
                interactive_condition = False
        # ************************************** #
        print("sma", sma_condition, ", sma rate: ", sma_rate_condition, ", macd: ", Formula.macd(self.close, 12, 26, 9), ", price: ", Formula.macd(self.close, 36, 78, 27), ", inter: ", interactive_condition)
